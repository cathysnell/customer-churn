...
files, Unity Catalog runs the DDL. Nothing here talks to Databricks — the
generator must run on a laptop with only numpy/pandas/pyarrow.
"""
...
def unity_catalog_ddl(config: GeneratorConfig) -> str:
    ...
    lines: list[str] = [
        ...
        f"-- Seed {config.seed} | {config.n_users} users | {config.months} months "
        f"| window {config.window_start.date()} .. {config.window_end.date()}",
        "",
